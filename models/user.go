...
var (
	userAllColumns            = []string{"user_id", "name", "age"}
...
// Divisions retrieves all the division's Divisions with an executor.
func (o *User) Divisions(mods ...qm.QueryMod) divisionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`division`.`user_id`=?", o.UserID),
	)

	query := Divisions(queryMods...)
	queries.SetFrom(query.Query, "`division`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`division`.*"})
	}

	return query
}

// LoadDivisions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDivisions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`division`), qm.WhereIn(`division.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on division")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for division")
	}
	...
	if singular {
		object.R.Divisions = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.UserID {
				local.R.Divisions = append(local.R.Divisions, foreign)
				if foreign.R == nil {
					foreign.R = &divisionR{}
				}
...
// AddDivisions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Divisions.
// Sets related.R.User appropriately.
func (o *User) AddDivisions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Division) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UserID, rel.DivisionID}
	...
	if o.R == nil {
		...
		o.R.Divisions = append(o.R.Divisions, related...)
	}
