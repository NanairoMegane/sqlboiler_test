...
var BranchWhere = struct {
	...
	BranchID: whereHelperint{field: "`branch`.`branch_id`"},
	Name:     whereHelperstring{field: "`branch`.`name`"},
	UserID:   whereHelpernull_Int{field: "`branch`.`user_id`"},
}
...
var (
	branchAllColumns            = []string{"branch_id", "name", "user_id"}
	branchColumnsWithoutDefault = []string{"name", "user_id"}
...
func FindBranch(ctx context.Context, exec boil.ContextExecutor, branchID int, selectCols ...string) (*Branch, error) {
	...
	query := fmt.Sprintf(
		"select %s from `branch` where `branch_id`=?", sel,
	)
...
func (o *Branch) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `branch` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `branch` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, branchPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for branch")
	}
...
func (o *Branch) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for branch")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for branch")
	}
...
func BranchExists(ctx context.Context, exec boil.ContextExecutor, branchID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `branch` where `branch_id`=? limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, branchID)
	}
